...
function init () {
	...
	$('#badge [type="checkbox"]').on('change', function () {
		...
		loadimage('pic/badgeSS6/' + badgeSS6[value].card_url);
	});
	...
	if ($('#div-top img[title="SS6 default 01"]')[0]) $('#div-top img[title="SS6 default 01"]').trigger('click');
...
function drawCanvas (code) {
	...
	for (var i = 1; i <= 4096; i *= 2) {
		if ((badge_get | i ) == badge_get) {
			junk.append($('<img id="junk-badge-' + i + '" src="pic/badgeSS6/' + badgeSS6[i].card_url + '"/>'));
...
function encode(text) {
    ...
    var lib_shuffle = "\|pSj\#sbh\/\*\!q\(F\@YPoW\~\.d7\[\;x8i\,\_\+c\?0A6RZ\]H\:\>a\=J\`T2Glz\^\"D\}\%\<r\)mw\{X\$\'1EOnB\\N\&Ck3feMytIQ\-9KgU45Lu"
